...
class GlacierFlowline(object):
    def __init__(self, nx, dx, **kwargs):
        ...
        self.sediment_entrainment_k = 5e-4
        self.deposition_beta = 2
        self.deposition_k = 1e-1
        ...
        self.at_node['sediment_flux'] = np.zeros(self.num_of_nodes)
        self.at_node['sediment_thickness'] = np.zeros(self.num_of_nodes)

        self.at_node['sediment_ice_thickness'] = np.zeros(self.num_of_nodes)
        self.at_node['sediment_ice_thickness'][self.core_nodes] = kwargs.get('sediment_ice_thickness', np.zeros(self.nx)+10)

        self.sediment_porosity = 0.35

        self.bedrock_roughness_scale = kwargs.get('bedrock_roughness_scale', 2)

        self.at_node['glacial_erosion_rate'] = np.zeros(self.num_of_nodes)
        self.at_node['sediment_entrainment_rate'] = np.zeros(self.num_of_nodes)
        self.at_node['sediment_deposition_rate'] = np.zeros(self.num_of_nodes)
    ...
    def update_sediment(self, dt):
        thk = self.at_node['thk']
        E_r = self.at_node['glacial_erosion_rate']
        p = self.at_node['sediment_thickness'] / self.bedrock_roughness_scale
        p[p > 1] = 1
        sliding_vel_not_stagged = self._unstagger(self.at_link['sliding_vel']) * self.secperyr

        E_s = np.power(np.abs(sliding_vel_not_stagged), self.erosion_l) * self.sediment_entrainment_k * p
        E_s[thk < 1e-3] = 0

        transport_layer_thk = self.at_node['sediment_ice_thickness'].copy()
        transport_layer_thk[transport_layer_thk > thk] = thk[transport_layer_thk > thk]
        transport_layer_flux = np.abs(sliding_vel_not_stagged) * transport_layer_thk
        transport_layer_flux[transport_layer_flux <= 0] = 1e-9
        q_s = self.at_node['sediment_flux']
        c_sed = np.abs(q_s) / transport_layer_flux
        
        D_s = np.zeros(self.num_of_nodes)
        D_s[c_sed <= 1] = self.deposition_k * (np.exp(self.deposition_beta * c_sed[c_sed <= 1]) - 1)
        D_s[c_sed > 1] = self.deposition_k * (np.exp(self.deposition_beta * 1) - 1)
        D_s_limit = np.abs(q_s) / self.dx + E_s + E_r
        D_s[D_s > D_s_limit] = D_s_limit[D_s > D_s_limit]
        
        D_s[thk < 1e-3] = 0

        self.at_node['sediment_thickness'] += dt * (D_s - E_s) / (1-self.sediment_porosity)
        
        self.at_node['surf'] = self.at_node['topg'] + self.at_node['sediment_thickness'] + self.at_node['thk']

        self.at_node['sediment_entrainment_rate'] = E_s
        self.at_node['sediment_deposition_rate'] = D_s
        
        # update sediment flux
        # find direction
        receiver = np.zeros(self.num_of_nodes, dtype=int) - 1
        surf_elev = self.at_node['surf']
        for i in self.core_nodes:
            if thk[i] > 1e-3:
                receiver[i] = i
                if surf_elev[i] > surf_elev[i+1] and thk[i+1] > 1e-3:
                    receiver[i] = i+1
                elif surf_elev[i] > surf_elev[i-1] and thk[i-1] > 1e-3:
                    receiver[i] = i-1
        # find head
        is_head = np.ones(self.num_of_nodes)
        for i in self.core_nodes:
            if receiver[i] != -1:
                is_head[receiver[i]] = 0
        is_head[thk < 1e-3] = 0

        # build an ordered stack
        in_list = np.zeros(self.num_of_nodes, dtype=int)
        ordered_nodes = np.zeros(self.num_of_nodes, dtype=int)
        stack_size = 0
        for i in self.core_nodes:
            if is_head[i]:
                ordered_nodes, stack_size = _add_to_stack(i, receiver, ordered_nodes,
                                                          stack_size, in_list)
        ...
        # update sediment flux
        sed_flux_grad = E_s + E_r - D_s
        q_s = np.zeros(self.num_of_nodes)
        for i in range(len(ordered_nodes)):
            sed_flux_change = self.dx * 0.5 * (sed_flux_grad[ordered_nodes[i]] + sed_flux_grad[receiver[ordered_nodes[i]]])
            q_s[receiver[ordered_nodes[i]]] += q_s[ordered_nodes[i]] + sed_flux_change
            if q_s[receiver[ordered_nodes[i]]] < 0:
                q_s[receiver[ordered_nodes[i]]] = 0 # caution: this may cause conservation problem!

        self.at_node['sediment_flux'] = q_s
    ...
    def update_topg(self, dt):
        ...
        p[p > 1] = 1
        sliding_vel_not_stagged = self._unstagger(self.at_link['sliding_vel']) * self.secperyr
        glacial_erosion_rate = np.power(np.abs(sliding_vel_not_stagged), self.erosion_l) * self.erosion_k * (1-p)
        glacial_erosion_rate[self.at_node['thk'] < 1e-3] = 0
        self.at_node['glacial_erosion_rate'] = glacial_erosion_rate

        self.at_node['topg'] -= dt * glacial_erosion_rate
    ...
    def update_model_state(self):
        ...
        self.model_state['sediment_thickness'].data = self.at_node['sediment_thickness'][self.core_nodes]
        ...
        self.model_state['sediment_entrainment_rate'].data = self.at_node['sediment_entrainment_rate'][self.core_nodes]
        self.model_state['sediment_deposition_rate'].data = self.at_node['sediment_deposition_rate'][self.core_nodes]
...
def _add_to_stack(i,
    ...
    if in_list[i]:
        return ordered_nodes, stack_size
    if receiver[i] != i:
        ordered_nodes, stack_size = _add_to_stack(receiver[i],
                                                  receiver, ordered_nodes,
                                                  stack_size, in_list)

    ordered_nodes[stack_size] = i
    stack_size += 1
    in_list[i] = True
